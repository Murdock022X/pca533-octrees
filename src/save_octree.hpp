...
inline void writeOctreeGroup(HighFive::File &out, const std::string &groupName,
  ...
  cstone::nodeFpCenters<KeyType>(
      std::span<const KeyType>(oct.prefixes.data(), oct.prefixes.size()),
...
collectOctreeFromViewCpu(const cstone::OctreeView<const KeyType> &view,
                         std::span<const KeyType> leaves) {
  constexpr size_t levelRangeSize = cstone::maxTreeLevel<KeyType>{} + 2;
  return {std::vector<KeyType>(leaves.begin(), leaves.end()),
          std::vector<KeyType>(view.prefixes, view.prefixes + view.numNodes),
          std::vector<cstone::TreeNodeIndex>(view.childOffsets,
                                             view.childOffsets + view.numNodes),
          std::vector<cstone::TreeNodeIndex>(
              view.internalToLeaf, view.internalToLeaf + view.numNodes),
          std::vector<cstone::TreeNodeIndex>(view.levelRange,
                                             view.levelRange + levelRangeSize)};
}

inline OctreeHostData collectFocusOctreeCpu(
    const cstone::Domain<KeyType, Real, cstone::CpuTag> &domain) {
  return collectOctreeFromViewCpu(domain.focusTree().octreeViewAcc(),
                                  domain.focusTree().treeLeaves());
}

inline OctreeHostData collectGlobalOctreeCpu(
    const cstone::Domain<KeyType, Real, cstone::CpuTag> &domain) {
  auto view = domain.globalTree();
  return collectOctreeFromViewCpu(
      view, std::span<const KeyType>(view.leaves, view.numLeafNodes + 1));
}
