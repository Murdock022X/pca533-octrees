...
void runner(HighFive::File& file, std::string group_name, int rank, int numRanks, bool gpu)
{
    ...
    std::cout << "Dataset loaded [" << group_name << "] -> n = " << ix.size() << "rank = " << rank << " with range [" << start << ", " << end << ")" << std::endl;
    ...
    if (!gpu)
    {
        runnerCpu(keys, ix_local, iy_local, iz_local, h, px_local, py_local, pz_local, rank, numRanks, bucketSize, bucketSizeFocus, theta, group_name);
    }
    else
    {
        runnerGpu(keys, ix_local, iy_local, iz_local, h, px_local, py_local, pz_local, rank, numRanks, bucketSize, bucketSizeFocus, theta, group_name);
    }
}
